...
5. **People-multiplier plumbing through aggregation.** Thread the plan's multiplier into the flow graph. `buildProductFlowGraph` and `buildPullLists` live in `src/lib/aggregation.ts` (not `builders/`). Add a `peopleMultiplier` argument to `buildProductFlowGraph` (called at `Outputs.tsx:320`) and multiply it into `mealCount` at the **three true derivation sites**, so `mealCount = (meal.quantity || 1) * peopleMultiplier`:
   ...
   **Pull lists — deliberate exclusion (Proposed).** `buildPullLists` (`aggregation.ts:101-158`, called at `Outputs.tsx:376`) does **not** scale by meal quantity today — it emits `quantity: node?.quantity` verbatim (`aggregation.ts:138`) with no `mealCount` at all. So per-meal `quantity` currently has zero effect on pull-list output. This phase therefore **excludes** pull lists from the people-multiplier: applying it there would introduce scaling behavior the per-meal `quantity` knob does not have and break the "equals hand-multiplying every meal's quantity" equivalence. If pull-list scaling is wanted, it requires a **preceding** fix that makes `buildPullLists` honor `meal.quantity` first (see Risks); that is out of scope for this item.

   **Threading the plan record.** `people_multiplier` lives on the `weekly_plans` record. The full plans list is already loaded at `Outputs.tsx:159` (`getAll(collections.weeklyPlans)`); the meal-fetch block at `Outputs.tsx:195` only fetches `plannedMeals` (filtered by `weekly_plan`) and does not have the plan record in scope. Look up the selected plan by `selectedPlanId` from the already-loaded plans list (or thread the plan object) so `people_multiplier` is available at the `buildProductFlowGraph` call site.
   - *Verify:* Setting a plan's multiplier to 2 doubles every shopping-list quantity, prep amount, and container count versus multiplier 1, and matches editing each meal's `quantity` ×2. **Pull-list quantities are unchanged by the multiplier (excluded by design).**
...
7. Setting a plan's **people-multiplier** to N scales the **meal-quantity-dependent outputs** — shopping-list quantities, batch-prep amounts, and container counts — by N relative to multiplier 1, and equals the result of multiplying every meal's `quantity` by N by hand. **Pull lists are excluded:** pull-list quantities do not depend on per-meal `quantity` today (`buildPullLists` emits unscaled `node.quantity`), so the multiplier does not change them and the hand-multiply equivalence does not apply to them (see item 5 and Risks).
...
- **Pull lists ignore per-meal quantity today (Open).** `buildPullLists` emits `node.quantity` unscaled (`aggregation.ts:138`) — neither per-meal `quantity` nor the new people-multiplier reaches it. This phase excludes pull lists from the multiplier (see item 5 / AC#7) rather than silently introducing scaling the per-meal knob lacks. **Open question:** should pull-list quantities scale with the week? If yes, a preceding fix must make `buildPullLists` honor `meal.quantity` (formula: `node.quantity × (meal.quantity || 1) × peopleMultiplier`), after which the multiplier and the AC#7 equivalence can be extended to pull lists. Deferred pending a decision.
