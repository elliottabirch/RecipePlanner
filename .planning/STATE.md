...
- `mis-tagged-just-in-time-timing-on-make-ahead-steps` — **CORRECTED 2026-07-16 (u4p): the retag was already applied by hand — no commit, no script, unrecorded actor.** Bourguignon's `Brown mushrooms`, `Simmer bourguignon`, and `Pull garlic cubes` all verified `batch` in prod on 2026-07-16; the edge (`Pull garlic cubes → Simmer bourguignon`) survives intact. 24 published JIT steps remain (not 29 — the difference is exactly the 5 retagged records), and the retag is unrecorded: `git log --all` over `*retag*`/`*timing*` is empty. `create spaghetti` was retagged `batch` WHOLESALE rather than split — the failure moved from invisible to *wrongly-scheduled on Saturday* — and `260716-u4p` Task 3 ships the split behind a gated write (checkpoint pending human apply). Stays pending for: (1) the human `--apply` of the split, and (2) tightening the import skill's guidance. See `.planning/todos/pending/2026-07-12-mis-tagged-just-in-time-timing-on-make-ahead-steps.md`.
