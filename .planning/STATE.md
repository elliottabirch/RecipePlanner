...
- `pull-step-names-drift-across-recipes` — **NEW 2026-07-17 (260717-fva).** The same freezer/pantry pull (`garlic cubes (frozen)`) is authored under three drifted names across recipes (`Pull garlic cubes` / `Pull out garlic cubes` / `Pull frozen garlic cube`). Explicitly filed as a RED HERRING for the 260717-fva bugs: both the week-graph merge and the missing-pull-step exemption key off the input product id / product type, never the step name, so normalizing names fixes neither. Kin to `swap-aware-prep-naming`. See `.planning/todos/pending/2026-07-17-pull-step-names-drift-across-recipes.md`.
