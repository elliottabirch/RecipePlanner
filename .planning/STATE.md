...
- `mis-tagged-just-in-time-timing-on-make-ahead-steps` — **DATA bug (user was right, my first read was wrong).** Bourguignon's `Brown mushrooms` + `Simmer bourguignon` and spaghetti's `create spaghetti` are tagged `just_in_time` but "simmering" is the skill's canonical *batch* example → they vanished from cook mode. Audit of 29 published JIT steps: most are correct (sear/bake-fish/plate); only these 4 are wrong. `Pull garlic cubes` must retag WITH the simmer or the edge silently drops. `create spaghetti` is a mega-step conflating batch sauce + day-of noodles — needs SPLITTING, not retagging. Also 17 prep steps carry a timing (skill says omit). See `.planning/todos/pending/2026-07-12-mis-tagged-just-in-time-timing-on-make-ahead-steps.md`.
- `day-of-steps-have-no-surface-cook-mode-ends-silently` — **Cook mode ended with 2 dinners unmade.** `week-graph.ts:151` drops every `just_in_time` step (`if (step.timing === Timing.JustInTime) continue;`), so cook mode can never surface them — and NO other screen shows JIT *steps* (`buildPullLists` shows only their *ingredients*). Verified in prod: Mushroom Bourguignon has 3 batch steps shown and 5 JIT steps hidden (incl. a 35-min braise); Meat Spaghetti's entire cook is 1 JIT step. The step data is CORRECT — day-of work is real; the app just has no day-of surface. Fix 1 (small): stop cook mode ending silently. Fix 2: build a day-of cook view. See `.planning/todos/pending/2026-07-12-day-of-steps-have-no-surface-cook-mode-ends-silently.md`.
...
- `alias-units-break-cross-recipe-aggregation` — **Silent under-counting app-wide.** The aggregation read path never calls `normalizeUnit`, so `nodeUnit` stays a raw alias (`"cube"`). `"cube"` isn't in `UNIT_DIMENSIONS` (15 canonical keys only) → `getDimension` is `undefined` → `canConvert("cube","cube") === false` → `resolveMergeTargetKey` splits to `${productId}|undefined` instead of merging. Garlic used in 2 recipes → 2 lines, only 1 surfaced. Affects ALL 17 aliases incl. `ea`. Also skips the discrete ceil. **Must land with `garlic-cube-clove-unit-conversion`** — fixing this alone merges cloves+cubes at a wrong 1:1 ratio. See `.planning/todos/pending/2026-07-12-alias-units-break-cross-recipe-aggregation.md`.
...
- `garlic-cube-clove-unit-conversion` — app says pull 3 garlic cubes where it should be 1: `clove` and `cube` BOTH alias to `each` (`units.ts:87-105`) and `convert()` is identity for count units, so the 3:1 ratio has nowhere to live. Modeling gap, not arithmetic. Needs a data sweep + a product-level pack/unit model; overlaps `single-purchase-unit-shopping-lines`. See `.planning/todos/pending/2026-07-12-garlic-cube-clove-unit-conversion.md`.
