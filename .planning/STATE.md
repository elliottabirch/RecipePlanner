...
status: executing
stopped_at: "Session shipped THREE things (all deployed): (1) E3 `usda-search-plain-rename` (260718-upn) — plain-name the Search-USDA PREFILL only (not the 7.8k index; user chose), `plainNameFromUsda` in its own JSON-free module to preserve the lazy chunk; optional prod `Kumquats, raw`→`kumquat` rename SCRIPTED but user-run-pending (classifier blocks prod writes). (2) Group E1 layer 1 (260718-e1a): derived step titles from a controlled `prep_action` vocab — `src/lib/prep-actions.ts` + `deriveStepLabel`, wired cook mode + batch prep + editor; prod migration APPLIED (99 steps set, 0 legacy left); resolves `pull-step-names-drift`. Cook mode user-verified; batch-prep/editor tablet-verify pending. (3) E1 design proposal doc with LOCKED decisions. NEXT: E1-b (layer-2 OUTPUT-product names, graph-time derivation — NOT the schema `base_product` field, which wouldn't be swap-aware) is DESIGNED + DEFERRED (user 2026-07-18, 'verify first'); `swap-aware-prep-naming` + `connective` thread-3 left PARTIAL in pending. Group F still deferred (other-server prereq). E3 `improve-recipe-tagging` (non-code, manual) untouched."
last_updated: "2026-07-18T13:15:00.000Z"
progress:
  total_phases: 6
  completed_phases: 6
  total_plans: 56
...
| Phase 06 P11 | 6min | 3 tasks | 5 files |
...
- `authoring-skills-need-downstream-consequences-reference` — **Upstream parent of most 2026-07-12 data bugs.** The recipe skills say what to write but not what each choice DOES: `raw` is the ONLY type ever shopped for; `just_in_time` steps are excluded from the week graph so their instructions are unreachable from every screen; a JIT producer feeding a batch consumer silently drops the edge; alias units silently split into invisible lines. Needs one shared `authoring-consequences.md` all three recipe skills read before emitting a graph, PLUS linter teeth (the timing convention was already documented and still got imported wrong). See `.planning/todos/pending/2026-07-12-authoring-skills-need-downstream-consequences-reference.md`.
- `mis-tagged-just-in-time-timing-on-make-ahead-steps` — **CORRECTED AGAIN 2026-07-17 (u4p): the split model itself was wrong, per a direct user interview.** The spaghetti is boiled on prep day, combined with the sauce, and stored combined — there is no day-of work for this dish. `create spaghetti` retagged `batch` WHOLESALE was therefore **correct**, not a half-fix; the todo's own "batch sauce + day-of plate" proposal was a misreading of how the user cooks and is retracted. The real, narrow defect was granularity: one 8a/15p mega-step hid two overlappable passive windows (real total 10a/23p). `260716-u4p` Task 1 rewrote `split-create-spaghetti-step.js` for a 4-step all-`batch` model (Cook spaghetti, Cook meat, Simmer meat sauce, Combine all), rehearsed end-to-end (apply + executed rollback) on `:8091` — the overlap is expressible only because prod's `scheduler_config.burner_count` is 2. Bourguignon's retag remains verified `batch` in prod, unrecorded (no commit, no script, unrecorded actor): `Brown mushrooms`, `Simmer bourguignon`, `Pull garlic cubes`, with the `Pull garlic cubes → Simmer bourguignon` edge intact. 24 published JIT steps remain (not 29). Stays pending for: (1) the human `--apply` of the split, and (2) tightening the import skill's guidance. See `.planning/todos/pending/2026-07-12-mis-tagged-just-in-time-timing-on-make-ahead-steps.md`.
- `day-of-steps-have-no-surface-cook-mode-ends-silently` — **CORRECTED 2026-07-16 (u4p): cook mode was not ending silently — it asserted "All steps complete!" (worse), and piece 1 (the message) SHIPPED.** `CookMode.tsx:704-712` rendered an affirmative completion claim while day-of work remained; `260716-u4p` Task 1 replaced it with a named list of remaining meals/steps via `day-of-work.ts`'s `collectDayOfWork`/`formatDayOfSummary`. Verified counts corrected: bourguignon is now 5 batch / 2 JIT (not 3/5) after the by-hand retag. **Piece 2 — a day-of cook surface — stays pending**; `PullListsTab` is NOT a substitute (dark for any week where all meals are week-spanning with no `day` set, which is 18/18 in week 7/13). See `.planning/todos/pending/2026-07-12-day-of-steps-have-no-surface-cook-mode-ends-silently.md`.
- `unproduced-non-raw-inputs-are-invisible` — **Soba peanut sauce: two symptoms, one bug.** A non-`raw` input that no step in the plan produces falls through every guard: `week-graph.ts` leaves its consumer as a t=0 graph SOURCE (deliberately — it delegates to the linter), `buildShoppingListFromFlow` includes only `ProductType.Raw`, `checkInventoryStock` inspects only `Inventory`, and `missing-pull-step` covers only stored/inventory AND runs on-demand in cook mode only. So combine got scheduled before the sauce existed, and nothing said whether to buy or make it. `Product.source_recipe`/`store_bought_product` already answer make-vs-buy — they just never reach the shopping list. First step: check the peanut sauce product's actual `type`. See `.planning/todos/pending/2026-07-12-unproduced-non-raw-inputs-are-invisible.md`.
- `alias-units-break-cross-recipe-aggregation` — **CORRECTED 2026-07-16: the "never calls normalizeUnit" diagnosis is retracted; normalize shipped in `997c74c`.** The live split is `""`-driven (D-01 sentinel), not alias-driven — zero alias units exist in prod today. Latent for 14 products with a mixed unresolvable+canonical unit spread; not firing on week 7/13's shopping list. The claim that this must land alongside the (now-resolved) garlic data fix is dropped — cloves/cubes cannot mis-merge today since every live garlic node already uses `each`. Deferred: an order-independent merge-semantics fix, pinned by `product-builder.test.ts`'s two-ordering test. See `.planning/todos/pending/2026-07-12-alias-units-break-cross-recipe-aggregation.md`.
- `publish-gate-is-severity-blind` — **NEW 2026-07-16 (u4p).** `RecipeEditor.tsx:767`'s `handlePublish` gates on `lintFindings.length > 0` — a `warning` blocks publish exactly like an `error`, so `LintFinding.severity` is never actually read. Scoped honestly: only 2/67 recipes would block on re-publish today, 0 drafts exist in prod — a design defect with a long tail, not a live outage. Surfaced while designing `timing-coherence`, which had to be tuned to 0 false positives specifically because a `warning` here is not advisory. See `.planning/todos/pending/2026-07-16-publish-gate-is-severity-blind.md`.
- `full-recipe-text-on-cook-mode-card` — button on the cook card to read the whole recipe in prose. `Recipe.notes` already exists and is plumbed end-to-end (editor + import contract + write path) — this is a content/surfacing job, not a schema one. Needs: pull descriptions in at import, author distilled ones for self-created recipes. Constraint: merged-prep cards carry a synthetic step with an EMPTY `recipe` (spans recipes), same reason `AddNoteButton` is omitted there. See `.planning/todos/pending/2026-07-12-full-recipe-text-on-cook-mode-card.md`.
- `container-convergence-indicator-on-cook-card` — cook card should say "this gets combined with X, Y into one container" (soba noodles + cucumber + green onion). **No schema change needed**: `container_type` is a vessel *kind* and `meal_destination` is a regex-parsed *meal label* — neither means "same jar". The real signal is graph convergence on a shared downstream assembly step, derivable from existing edges. See `.planning/todos/pending/2026-07-12-container-convergence-indicator-on-cook-card.md`.
- `checkoff-annihilates-passive-time-in-retime` — **RESOLVED 2026-07-17 (260717-25d Task 1).** Fixed by flooring `retime.ts`'s passive derivation at its estimate (live line numbers `retime.ts:138-149`, not the drifted `:130-146`) instead of deriving it from elapsed time — a prompt check-off no longer deletes a step's passive window. Measured to be bigger than the original framing: the collapse also phantom-freed the resource (burner/oven) a checked-off step still physically occupies, letting the scheduler place a second pot on an occupied burner with no precedence edge at all. Genuine overrun absorption still works (verified: a real 50-minute simmer still ends at 50). Deferred `deriveReadiness` passive-awareness rider spun out to its own todo. See `.planning/todos/resolved/2026-07-12-checkoff-annihilates-passive-time-in-retime.md`.
- `cook-mode-list-shuffles-on-checkoff` — **RESOLVED 2026-07-17 (260717-25d Task 2).** Fixed by freezing the rendered display order at schedule generation (`display-order.ts`, extracted React-free per the `readiness.ts` precedent) instead of re-sorting `orderedByTime` by live `schedule.starts` on every retime. The order thaws ONLY on an explicit Regenerate (user decision, no heuristics); `handleToggleChecked` never touches it. Confirmed the residual shuffle this todo predicted survives Task 1's passive fix (a genuine overrun still crosses a neighbour) — pinned by `display-order.test.ts`. See `.planning/todos/resolved/2026-07-12-cook-mode-list-shuffles-on-checkoff.md`.
- `deprioritize-store-steps-in-cook-mode` — "store" steps cluster at the START of the cook list, ahead of the long-pole recipes. GA is indifferent to them (no fitness term; `resource:"none"` → no resource pressure) and raw-input store steps are dependency-free so SSGS schedules them at t=0. Blocker: no first-class marker — `StepType` is only Prep|Assembly and `prep_action` has no "store". User leans toward hiding them entirely. See `.planning/todos/pending/2026-07-12-deprioritize-store-steps-in-cook-mode.md`.
...
- `readiness-is-blind-to-running-passive-windows` — **RESOLVED 2026-07-17 (260717-pwr), as the model layer of the passive-window feature.** `deriveReadiness` now takes an optional `runningPassiveSet`; a checked-but-still-simmering producer no longer satisfies its dependents, and `ReadinessResult` splits blockers into `waitingOn` (unchecked) vs `simmering` (running) so the chip reads `MM:SS left on <producer>` instead of a premature "ready". Default-empty param keeps all prior callers/tests unchanged (4 new tests pin the passive path). See `.planning/todos/resolved/2026-07-17-readiness-is-blind-to-running-passive-windows.md`.
- `passive-windows-outlive-checkoff-need-a-surface` — **RESOLVED 2026-07-17 (260717-pwr), built as one feature with the readiness sibling above.** Persistent "Running now" strip in `CookMode.tsx` keeps a checked-off step's timer counting down (the `nowStartRef` anchors already survive check-off — the timer was computable all along, it just had nowhere to render); a window drops at 0:00. One tap, no alarm. User follow-up added a ✓ per row to finish a window early. Deployed to NAS + human-verified at the stove; commits cdc14c6 + b23899d. See `.planning/todos/resolved/2026-07-17-passive-windows-outlive-checkoff-need-a-surface.md`.
- `pull-step-names-drift-across-recipes` — **RESOLVED 2026-07-18 (260718-e1a).** Fixed by the E1 derived-step-label rework: titles derive `"{verb} {input}"` from a controlled `prep_action`, so all three drifted pull names collapse to one canonical `pull garlic cubes (frozen)` on every surface and the drift can't recur (authored `name` is no longer shown). See `.planning/todos/resolved/2026-07-17-pull-step-names-drift-across-recipes.md`.
...
- 260716-u4p: create-spaghetti 4-step all-batch split rewritten + rehearsed (apply + rollback) on :8091, but the human-gated prod --apply has not run — prod's `create spaghetti` mega-step still schedules the noodle boil and sauce simmer as one undivided 8a/15p step, not two overlappable ones (this is a granularity limitation, not a mis-scheduling bug — meat spaghetti has no day-of work either way, per the 2026-07-17 user interview)

### Quick Tasks Completed

| # | Description | Date | Commit | Directory |
|---|-------------|------|--------|-----------|
| 260718-e1a | Group E1 layer 1 — derive step titles from a controlled `prep_action` vocab instead of authored free-text. New single-source `src/lib/prep-actions.ts` (flat 20-key enum, size baked in, `{verb,state}` display forms) + `deriveStepLabel` (hybrid: derive `"{verb} {input}"` only where `prep_action` set + single input, else authored name — swap-aware because it reads the input node a swap re-points). Wired cook mode (StepInstance.displayName in week-graph, general non-resource-gated single-input resolver so `process broccoli` on the food-processor still derives), batch prep (threaded `prep_action` through AggregatedFlowStep + derived name in buildPrepSteps), and the recipe-editor canvas. Migration `scripts/apply-e1-prep-actions.mjs` (idempotent widen→backfill→narrow, NAME-first inference fixing coarse Phase-5 mis-tags: brunoise/chiffonade/zest/quarter were stored minced/shredded/grated/chopped) applied to test then **prod: 99 steps set, 3 stale legacy cleared, 0 legacy remaining**. Resolves `pull-step-names-drift` (all 3 garlic-cube pull names → one canonical `pull garlic cubes (frozen)`); partial on `swap-aware-prep-naming` + `connective` thread 3 (layer-1 titles done + swap-aware; layer-2 OUTPUT-product names = E1-b, deferred). Cook mode **deployed + user-verified**; batch-prep/editor deployed, tablet-verify pending. 385/385 green. | 2026-07-18 | fae37f4, f7126e8 | (proposal in `.planning/notes/`; pull-drift todo → resolved/) |
...
| 260716-u4p | Stop cook mode's false "All steps complete!" during day-of cooking; `timing-coherence` linter rule (0 FP against 24 live JIT steps); rewrote `create spaghetti` split for the user-confirmed 4-step all-`batch` model (no day-of work for this dish) behind a gated write, rehearsed (apply + rollback) on `:8091` — checkpoint pending human apply; backlog hygiene correcting the by-hand bourguignon retag process finding AND the split model itself (user interview 2026-07-17) | 2026-07-17 | c604b09 | [260716-u4p-jit-silent-end-and-mistag](./quick/260716-u4p-jit-silent-end-and-mistag/) |
| 260717-25d | Fixed `retimeSchedule`'s check-off passive collapse — a prompt check-off no longer phantom-frees an occupied burner (physically impossible schedules, no precedence edge needed); floored passive at its estimate, genuine overrun absorption preserved. Froze cook mode's rendered display order at generation (`display-order.ts`, React-free) so it thaws ONLY on an explicit Regenerate — the residual shuffle that survives the passive fix is gone. Both source todos resolved with their diagnoses confirmed; `deriveReadiness` passive-awareness spun out to its own honestly-titled todo. Pure code — no prod write, no deploy; the tablet human-check is deferred/unexercised. 333/333 tests green. | 2026-07-17 | 16b9c4f | [260717-25d-checkoff-retime-and-shuffle](./quick/260717-25d-checkoff-retime-and-shuffle/) |
| 260717-fva | Extended `buildWeekGraph`'s week-wide merge to single-inventory-input pulls (parallel to the existing raw-prep path, own `Pull {product}` label — never `makeMergedPrepStep`) so three per-recipe `garlic cubes (frozen)` pulls collapse into one cook-mode card. Exempted sourceless INVENTORY consumptions from `missing-pull-step` (legit prior-week/pantry stock) while keeping sourceless STORED consumptions flagged (rule keeps teeth) — live-verified against plan `71ukhycp2v4s0fw`: 9 remaining false positives post-merge, all cleared, all confirmed `productType=inventory`. Pure code, no prod write, no deploy. 340/340 tests green, every new test RED-checked (stash-confirmed) against the unfixed code. | 2026-07-17 | ddee263 | [260717-fva-merge-pulls-and-fix-missing-pull-lint](./quick/260717-fva-merge-pulls-and-fix-missing-pull-lint/) |
| 260718-uni | Surfaced consumed-but-unmade non-raw inputs across 4 surfaces (Group D part 2, `unproduced-non-raw-inputs`). A `transient`/`stored` input no in-plan step produces and that isn't prior stock (the `asian peanut dressing`) fell through every guard. Library-wide probe: exactly 1 real case, 0 false positives (intra-recipe production yields a week-graph edge for any type; transients never cross recipes). **Detection**: `collectStoredInputConsumptions` now includes transient, sourceless transient flagged (Inventory exempt), finding carries productId. **Cook-mode readiness**: red "blocked: nothing makes X" chip (new ReadinessChip state) via a continuous `blockedInfoById` week-lint memo. **Outputs**: new `UnmadeInputsSection` on the shopping list (parallel to OutOfStockSection) with Make-it/Buy-it (source_recipe/store_bought_product) or a red "fix this recipe" finding. **Publish gate**: recipe-scoped `unmade-transient` rule (optional graph arg into composeRecipeFindings/runRecipeLint) blocks publishing a recipe that consumes a transient it never makes/can't buy. Verified firing against real soba-recipe prod data. 356/356 green (10 new). **Deployed + user-confirmed.** Source todo → resolved. Data fix (add source_recipe/store_bought or a make-step) left to the user. | 2026-07-18 | dd80e37 | (todo → `.planning/todos/resolved/`) |
| 260718-wpl | Restored the week pull list on the Batch Prep print view (Group D part 1). New `buildWeekPullList(flowGraph)` sources from `flowGraph.products` (the all-nodes map the shopping list reads) instead of the batch-prep step array, so JIT-only + original-packaging ingredients reappear and non-raw prior stock is included; keeps every consumed product no in-plan step produces (excludes in-plan-produced + transient), sums instanced items, multiplier pre-baked. Print view groups by storage location with an explicit "Other" bucket (no more silent drop). `display:none` print mount + already-in-inventory marking deliberately deferred. 6 new tests, 350/350 green. **Deployed + user-confirmed the print view is populated.** Source todo → resolved. | 2026-07-18 | 56a3abf | (todo → `.planning/todos/resolved/`) |
| 260718-cca | Group A — cook-card information architecture (both sibling todos from one capture, designed once). **Convergence indicator**: new pure `deriveConvergence` (alongside `readiness.ts`) walks the recipe graph downstream — step → output node → consuming assembly step → its other inputs → shared container — to answer "what does my ingredient combine with, into what container." No schema change (signal already in the graph); co-inputs resolve to base names, destination prefers `container_type` else the stored product name. Rendered as a "Combines with: … → container" call-out in the card's tap-to-expand body. 8 tests mirror the real Peanut Soba Salad subgraph probed from prod. **Full-recipe reader**: 📖 button beside the note button opens a dialog with `Recipe.notes` prose + this week's scaled ingredient list; both omitted on merged-prep nodes. **Import skill**: notes contract strengthened to carry full source prose. 365/365 green, tsc+eslint clean, build green. Merged-prep nodes handled too (per-recipe convergence rows + multi-recipe reader dialog, follow-up `aac7a0a`). **Data pass**: authored tight glanceable prose (user style — base steps, result-based, no technique timings) for 31 recipes that had empty `notes` (56/67 were empty; trivial pull/store entries left to the ingredient-list fallback). 2 from user-supplied source (Creamy Tomato Soup, Shawarma Pitas), 29 curated from each step graph; written to prod via `recipe-planner/apply-recipe-notes.mjs` (exact-name, empty-only, dry-run-then-apply) from `recipe-notes-drafts.json`. Created gitignored `.env` + `.env.example` for PB superuser creds (seeds Group F `deploy-pb-superuser-env`). **Deployed + user-confirmed on tablet.** Both source todos → resolved. | 2026-07-18 | f5db1bc, aac7a0a | (todos → `.planning/todos/resolved/`) |
| 260717-pwr | Passive-window feature (surface + model, both halves of the `passive-windows-outlive-checkoff` / `readiness-is-blind-to-running-passive-windows` todo pair). `CookMode.tsx` gets a persistent "Running now" strip that keeps a checked-off step's simmer/bake counting down (reuses `getCountdown`'s `nowStartRef` anchors, which already survive check-off) so a timer never vanishes on tap; a window drops at 0:00. `deriveReadiness` takes an optional `runningPassiveSet` — a checked-but-still-simmering producer no longer flips its dependents to "ready"; the chip reads `MM:SS left on <producer>`. User follow-up: each strip row has a ✓ to finish a window early so a pot that reduced ahead of estimate stops blocking later steps. Session-local surfaces (reset on load/regenerate). **Deployed to NAS (bundle `index-Y4u253mY.js`), human-verified at the stove.** 344/344 tests green (4 new). | 2026-07-17 | b23899d | (both todos → `.planning/todos/resolved/`) |
