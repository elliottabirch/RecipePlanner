...
- `checkoff-annihilates-passive-time-in-retime` — **Cook mode suggests impossible next steps** (blend the tomato soup while 45min of simmer remains). `retime.ts:130-146` recomputes a checked step's footprint as `passive_minutes = max(0, actualElapsed - activeOcc)`, which is **0 whenever `actualElapsed <= estActive`** — i.e. any prompt check-off deletes the step's entire passive window, collapsing dependents' start times. `retime.ts` assumes elapsed spans active+passive; `CookMode.tsx` anchors on "Now" so elapsed only ever spans *active*. The no-anchor fallback path is correct; the measured path is wrong. See `.planning/todos/pending/2026-07-12-checkoff-annihilates-passive-time-in-retime.md`.
- `cook-mode-list-shuffles-on-checkoff` — **REGRESSION (different bug from 3665946).** Checking off a step in cook mode permutes the rendered list. `CookMode.tsx:10-14` promises the order "is never permuted", and `schedule.order` indeed isn't — but the list renders from `orderedByTime` (`:298-308`), which re-sorts by `schedule.starts`, and check-off calls `retimeSchedule` → `setSchedule` on every tap, recomputing those starts. Fix: sort once at generation time, not on every retime. See `.planning/todos/pending/2026-07-12-cook-mode-list-shuffles-on-checkoff.md`.
