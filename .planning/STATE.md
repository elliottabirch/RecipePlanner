...
- `readiness-is-blind-to-running-passive-windows` — **NEW 2026-07-17, spun out of `checkoff-annihilates-passive-time-in-retime`'s deferred rider (260717-25d Task 3).** `deriveReadiness` is purely `checked`-based, so a dependent reads "ready" the instant its predecessor is checked off, with no notion of a running passive window (e.g. 43 minutes still left on a simmer). `getCountdown` already computes the remaining passive time; readiness should consult it. Deferred deliberately (a chip-semantics change, i.e. a feature, not this bug) — but now MORE load-bearing, not less: the display order is now frozen, and the freeze's own rationale is that readiness chips (not order) are what tell the cook a step is startable. Overlaps with the sibling `passive-windows-outlive-checkoff-need-a-surface` todo — treat as one feature. See `.planning/todos/pending/2026-07-17-readiness-is-blind-to-running-passive-windows.md`.
- `passive-windows-outlive-checkoff-need-a-surface` — cross-references corrected 2026-07-17 (260717-25d): its table cited both now-resolved todos above as still open. Updated in place to point at their resolved locations and at the new `readiness-is-blind-to-running-passive-windows` todo. No scope change — this todo's own timer-surface work is unaffected and still pending. See `.planning/todos/pending/2026-07-17-passive-windows-outlive-checkoff-need-a-surface.md`.
